...
var items=document.getElementsByClassName('mover');function updatePositions(){frame++;window.performance.mark("mark_start_frame");var calc=document.body.scrollTop / 1250;var l=items.length
var phase=[];for(var i=0;i<5;i++){phase.push(Math.sin(calc+i)*100);}
for(var i=0;i<l;i++){items[i].style.left=items[i].basicLeft+phase[i%5]+'px';}
...
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=5;var s=256;var intFrameHeight=window.innerHeight/s*cols;for(var i=0;i<intFrameHeight;i++){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i / cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem);}
updatePositions();});